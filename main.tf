...
variable "subnetpub1" {
    type    = map
    default = { name = "public1", cidr = "10.0.1.0/24", az = "a" }
}

variable "subnetpub2" {
    type    = map
    default = { name = "public2", cidr = "10.0.2.0/24", az = "b" }
}

variable "subnetpriv1" {
    type    = map
    default = { name = "private1", cidr = "10.0.4.0/22", az = "a" }
}

variable "subnetpriv2" {
    type    = map
    default = { name = "private2", cidr = "10.0.8.0/22", az = "b" }
}
...
resource "aws_nat_gateway" "ngw1" {
    ...
    depends_on    = [ aws_internet_gateway.igw1, aws_subnet.private1, aws_subnet.private2 ]
}
...
resource "aws_subnet" "private1" {
  ...
  tags = { Name =  var.subnetpriv1["name"] }
}
...
resource "aws_subnet" "private2" {
  ...
  tags = { Name =  var.subnetpriv2["name"] }
}
...
resource "aws_network_acl" "priv_nacl1" {
  vpc_id     = aws_vpc.default.id
  subnet_ids = formatlist( "%s", ["${aws_subnet.private1.id}", "${aws_subnet.private2.id}"] )
...
resource "aws_network_interface_sg_attachment" "sg_attachment1" {
  ...
  depends_on = [aws_internet_gateway.igw1, aws_instance.bastion1]
}
...
resource "aws_network_interface_sg_attachment" "sg_attachment2" {
  ...
  depends_on = [aws_internet_gateway.igw1, aws_instance.bastion2]
}
...
variable "amis_os_map_owners" {
  ...
  type = map
   default = {
      ...
      "ubuntu-18.04s" = "099720109477", #CANONICAL
...
data "aws_ami" "image_bast" {
  ...
  owners = formatlist( "%s", [var.amis_os_map_owners["ubuntu-18.04"]])
}
...
data "aws_ami" "image_deb10" {
  ...
  owners = formatlist( "%s", [var.amis_os_map_owners["debian-10"]])
}
